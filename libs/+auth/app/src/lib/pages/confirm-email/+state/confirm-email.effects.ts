import { Injectable } from '@angular/core';
import { of } from 'rxjs';
import { switchMap, map, catchError, tap } from 'rxjs/operators';
import { Router } from '@angular/router';
import { createEffect, Actions, ofType } from '@ngrx/effects';

import { ConfirmEmailFacade } from './confirm-email.facade';
import * as ConfirmEmailActions from './confirm-email.actions';
import { AuthService } from '../../../services/auth.service';
import { Snackbar } from '@nui/shared-app/ui/snackbar';
import { AuthAppFacade } from '../../../+state/auth-app.facade';

declare let $localize: any;

@Injectable()
export class ConfirmEmailEffects {
  public readonly resendConfirmation$ = createEffect(() => {
    return this._actions$.pipe(
      ofType(ConfirmEmailActions.resendConfirmation),
      switchMap((action) => {
        return this._authService.resendRegisterConfirmation(action.dto).pipe(
          map((res) => {
            return ConfirmEmailActions.resendConfirmationSuccess();
          }),
          catchError((error) => {
            return of(ConfirmEmailActions.resendConfirmationFailure({ error }));
          })
        );
      })
    );
  });

  public readonly resendConfirmationFailure$ = createEffect(
    () => {
      return this._actions$.pipe(
        ofType(ConfirmEmailActions.resendConfirmationFailure),
        tap(() => {
          const message = $localize`Failed to resend the confirmation. Contact the system administrator.`;
          const icon = 'x';
          this._snackbar.open({ message, icon });
        })
      );
    },
    { dispatch: false }
  );

  public readonly resendConfirmationSuccess$ = createEffect(
    () => {
      return this._actions$.pipe(
        ofType(ConfirmEmailActions.resendConfirmationSuccess),
        tap((action) => {
          const message = $localize`A new confirmation email was sent to you.`;
          const icon = 'check';
          this._snackbar.open({ message, icon });
        })
      );
    },
    { dispatch: false }
  );

  constructor(
    private readonly _actions$: Actions,
    private readonly _router: Router,
    private readonly _authService: AuthService,
    private readonly _authAppFacade: AuthAppFacade,
    private readonly _snackbar: Snackbar
  ) {}
}
